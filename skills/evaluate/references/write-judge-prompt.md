...
Enforce structured output. If the provider doesn't support schema enforcement, specify the JSON schema in the prompt.
...
Once the judge prompt is designed, implement it as an Opik `GEval` metric with your custom criteria. Opik's LLM-as-judge metrics support configurable judge models:
...
tone_judge = GEval(
    name="tone_match",
    criteria="your judge prompt criteria here",
    model="gpt-4o",
)
```

Run the judge as part of an Opik experiment to score traces at scale and compare results across experiments.

## Model Selection

Start with the most capable model available. The same model used for the main task works as judge (the judge performs a different, narrower task). Optimize for cost later once alignment is confirmed. Opik's LLM-as-judge metrics accept a `model` parameter to configure which model serves as judge.
