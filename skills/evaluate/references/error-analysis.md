...
- Use the `generate-synthetic-data` reference or Opik's dataset AI Expansion feature
- Run synthetic queries through the full pipeline and capture complete traces in Opik
...
Go back through all traces and apply binary labels (pass/fail) for each failure category. Each trace gets a column per category. Use Opik's annotation queues to collect structured feedback, or export traces to a spreadsheet.
...
**For failures that warrant an evaluator:** prefer Opik's heuristic metrics (`Equals`, `Contains`, `RegexMatch`, `IsJson`, `JsonSchemaMatch`) for anything objective. Use `write-judge-prompt` only for failures that require judgment — Opik's `GEval` metric supports custom LLM-as-judge criteria. Critical requirements (safety, compliance) may warrant an evaluator even after fixing the prompt, as a guardrail.
