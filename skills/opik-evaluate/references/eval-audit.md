...
Access to eval artifacts (traces, evaluator configs, judge prompts, labeled data) via Opik. If none exist, skip to "No Eval Infrastructure."

## Connecting to Eval Infrastructure

Use Opik to pull traces, evaluator definitions, and experiment results. If the user hasn't instrumented their pipeline with Opik yet, help them set up tracing first.
...
**Finding if missing:** Evaluators built without error analysis measure generic qualities ("helpfulness", "coherence") instead of actual failure modes. Start with the `error-analysis` reference, or `generate-synthetic-data` first if no traces exist.
...
**Finding if not binary:** Likert scales are difficult to calibrate. Annotators disagree on the difference between a 3 and a 4, and judges inherit that noise. Convert to binary pass/fail with explicit definitions using `write-judge-prompt`.
...
Flag LLM judges used for objectively checkable criteria: format validation, constraint satisfaction, keyword presence, schema conformance. Opik provides heuristic metrics like `Equals`, `Contains`, `RegexMatch`, `IsJson`, and `JsonSchemaMatch` for these.

**Finding if over-relying on judges:** Replace objective checks with Opik's heuristic metrics or custom code-based metrics. Reserve LLM judges for criteria requiring interpretation.
...
**Finding if present:** These metrics measure surface-level overlap, not correctness. Replace with binary evaluators grounded in specific failure modes.
...
**Finding if unvalidated:** An unvalidated judge may consistently miss failures or flag passing traces. Measure alignment using TPR and TNR on a held-out test set. See the `validate-evaluator` reference.
...
**Finding if using accuracy:** With class imbalance, raw accuracy is misleading: a judge that always says "Pass" gets 90% accuracy when 90% of traces pass but catches zero failures. Use TPR and TNR, which map directly to bias correction. See `validate-evaluator`.
...
**Finding if leaking:** Using evaluation data as few-shot examples inflates alignment scores and hides real judge failures. Split into train (few-shot source), dev (iteration), and test (final measurement). See `validate-evaluator`.
...
**Finding if output-only:** Reviewing only the final output hides where the pipeline broke. Show the full trace in Opik: input, intermediate steps, tool calls, retrieved context, and final output.

**Check:** How is data displayed to reviewers?

Flag raw JSON, unformatted text, or spreadsheets with trace data in cells.

**Finding if raw format:** Reviewers spend effort parsing data instead of judging quality. Use Opik's annotation queues to present traces with structured feedback collection.
...
1. Start with `error-analysis` on a sample of real traces from Opik.
...
**Fix:** [Concrete action, referencing an evaluation reference]
```
