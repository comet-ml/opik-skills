...
### Step 6: Store in Opik Dataset and Run Through Pipeline

Store the generated queries in an Opik dataset using `client.get_or_create_dataset(name=..., project_name="my-project")`. Then execute all queries through the full LLM pipeline with Opik tracing enabled to capture complete traces: input, all intermediate steps, tool calls, retrieved docs, final output.

Datasets are project-scoped, so always pass `project_name`. If the pipeline also uses `@track` tracing, the `project_name` configured for tracing must match the one passed to `get_or_create_dataset`, or the dataset and traces will land in different projects.

Opik also offers a built-in **AI Expansion** feature that can generate additional dataset items from seed examples with configurable diversity.
